...
print("Query:")
print(" (transaction_id = 1) UPDATE course SET coursename = 'Sistem Basis Data' WHERE year >= 2023 AND year <= 2024;")
...
ret_1 = DataRetrieval(table=["course"], column=[], conditions=[
    Condition(column="year", operation=">=", operand=2023, connector=None),
    Condition(column="year", operation="<=", operand=2024, connector="AND"),
])
...
print("--Trying FailureRecovery.write_log() (UPDATE operation):\n")
print("Query:")
print(" (transaction_id = 1) UPDATE course SET coursename = 'Pemrograman Berbasis Objek' WHERE year >= 2023 AND courseid <= 2024;")
...
for row in data_before:
    new_row = row.copy()
    new_row['coursename'] = 'Pemrograman Berbasis Objek'
    data_after.append(new_row)
