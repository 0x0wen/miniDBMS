...
def getWhereNode(tree: QueryTree): # type: ignore
    ...
    for child in tree.children:
        if child.node_type == "WHERE" and "." not in child.val[2]:
            if child.children:
                listChildren += getWhereNode(child) + [{"Column": child.val[0].split(".")[0], "value": child.val}]
            else:
    ...
    for child in tree.children:
        if child.node_type.lower().startswith("value"):
            if child.val[0].split(".")[0] in [node["Column"] for node in whereNode]:
                whereChildNode = QueryTree("WHERE", [node["value"] for node in whereNode if node["Column"] == child.val[0]], newTree)
                whereChildNode.children.append(child)
                newTree.children.append(whereChildNode)
        else:
            if (child.node_type == "WHERE" and child.val[0].split(".")[0] in [node["Column"] for node in whereNode]):
                newTree.children.append(buildNewTree(child, whereNode))
            else:
                newTree.children.append(buildNewTree(child, whereNode))
    return newTree

def optimizeWhere(query: ParsedQuery):
    tree = query.query_tree

    whereNode = getWhereNode(tree)

    print("++++++++Where Optimization++++++++")
    print(whereNode)
    print("++++++++Where Optimization++++++++")
